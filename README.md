...
| **humanise-prose** | `humanise-prose-plugin` | Remove AI writing tells from prose so it reads as human-authored, with a taxonomy of tells and realistic typo patterns for when polish itself is the giveaway. |
...
| **merman** | `merman-plugin` | Render Mermaid diagrams locally with merman-cli, a Rust port of Mermaid.js with no browser, Node, or Chromium. Outputs SVG, PNG, JPG, PDF, or terminal ASCII/Unicode, deterministically. |
| **op-cli** | `op-cli-plugin` | Experimentally verified traps of the 1Password CLI when driven from code: silent no-op edits under uvloop (non-FIFO stdin), field-blanking round-trips, service-account quirks, and keeping secrets out of argv. |
| **photo-upscale-for-print** | `photo-upscale-for-print-plugin` | Upscale a scan or low-resolution photo for poster or mural print: ppi maths, a model bake-off on a crop, a grain-aware finishing chain, numerical verification, and the Real-ESRGAN reimplementation traps. |
...
| **python-desktop-packaging** | `python-desktop-packaging-plugin` | Package Python desktop apps (PyInstaller, PyWebView, Briefcase, py2app) for macOS, Linux, and Windows. Covers the self-test contract, spec traps, CI runner choice, SBOMs, attestations, and honest signing. |
